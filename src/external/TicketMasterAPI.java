...
public class TicketMasterAPI {
	...
	private static final String API_KEY = "txVGxklU8S8ZPTdFIqVPEqKQ3sGM12Q3";
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			...
			int responseCode = connection.getResponseCode();  // send request and get the responsecode
		...
		if(!event.isNull(EMBEDDED)) {
			...
			if(!embedded.isNull(VENUES)) {
				...
				for(int i = 0; i < venues.length(); i++) {
					...
					if(!addr.equals("") /* or !addr.isEmpty() */) {
						return addr;
					}
		...
		if(!event.isNull(IMAGES)) {
			...
			for(int i = 0; i < images.length(); i++) {
				...
				if(!image.isNull(URL_STR)) {
					String url = image.getString(URL_STR);
					return url;
				}
			}
		}
